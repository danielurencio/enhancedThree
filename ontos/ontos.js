...
City.prototype = {
	...
	renderPoints: function(p,G,M) {
	  ...
	  for(var i in p) {
	    var particle = new Object;//THREE.Vector3(p[i].coords[0],p[i].coords[1],0);
	    ...
	    conteiner.push(particle);
	};
	this.particles = conteiner;
	},
	...
	dynamics: function(T) {
	  ...
	  var population = particles.map(function(i) { return i.inHouse; })
			    .reduce(function sum(a,b) { return a + b; }, 0);
	...
	receive: function(p) {
	  ...
	  this.particles.vertices[p.n].inHouse = +p.r.inHouse;
...
module.exports = { City: City, Agent:Agent, Time:Time };
